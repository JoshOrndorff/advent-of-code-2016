...
def decrypt(cipher, shift):
  ...
  for char in cipher:
    if char == '-':
      ...
      cipherNum = ord(char) - ord('a')
      plainNum = (cipherNum + shift) % 26
      plain += chr(plainNum + ord('a'))
...
# Now decrypt each room name
for room in rooms:
  realName = decrypt(room[0], room[1])
  
  # Checking for the room in question
  if "north" in realName:
    print(realName + ": " + str(room[1]))
