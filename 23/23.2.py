...
def execute(inst):
  ...
  # Don't execulte blank lines (used to preserve relative jumps)
  if inst == "":
    return
  ...
  # My custom multiply instruction
  if op == "mul":
    mul(x, y)
    return
  ...
  # Sanity check and debugging help
  print(inst)
...
def mul(x, y):
  """ My custom multiply instruction. Multiplies the two values given (as far as
  I can tell, always registers b and d) and stores the result in register a. """
...
with open("input-mul.txt", 'r') as f:
